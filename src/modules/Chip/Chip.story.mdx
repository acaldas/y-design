import { Meta, Story, Preview } from '@storybook/addon-docs/blocks';
import { H3 } from "../Typography/Typography";
import BaseChip from "./BaseChip";
import StatusChip from "./StatusChip";
import CloseableChip from "./CloseableChip";
import {ChipIcon,DerivedCloseableChip} from "./Chip.story";
import TestWrapper from "../Test/Test";

<Meta title="Components| Chip" />

# Chip

Chips are compact elements that represent an input, attribute, or action.is the

### TEST

<Preview>
  <Story name="TEST">
    <div style={{display: "flex", justifyContent:"space-around", alignItems: "center"}}>
        <TestWrapper></TestWrapper>
    </div>
  </Story>
</Preview>

### BaseChip

<Preview>
  <Story name="BaseChip">
    <div style={{display: "flex", justifyContent:"space-around", alignItems: "center"}}>
        <BaseChip>chip</BaseChip>
        <BaseChip onClick={() => console.log("click")}>clickable</BaseChip>
        <BaseChip selected>selected</BaseChip>
        <BaseChip disabled>disabled</BaseChip>
    </div>
  </Story>
</Preview>


### Left/Right Icons

The chip can be customized using _renderLeftIcon_ or _renderRightIcon_.


<Preview>
  <Story name="IconChip">
    <div style={{display: "flex", justifyContent:"space-around", alignItems: "center"}}>
      <BaseChip renderLeftIcon={() => <ChipIcon type="left" name="add"/>}>
         with left icon
      </BaseChip>
       <BaseChip renderRightIcon={() => <ChipIcon type="right" name="add"/>}>
         with right icon
       </BaseChip>
       <BaseChip renderLeftIcon={() => <ChipIcon type="left" name="add"/>}
                 renderRightIcon={() => <ChipIcon type="right" name="arrow"/>}>
          with both icons
      </BaseChip>
    </div>
  </Story>
</Preview>

### StatusChip

Props:
- type: string


There are 4 possible types for any chip:<br/>
__default__ __success__ __warning__ __danger__

<Preview>
  <Story name="StatusChip">
    <div style={{display: "flex", justifyContent:"space-around", alignItems: "center"}}>
      <StatusChip type="default">default</StatusChip>
      <StatusChip type="default" selected>default</StatusChip>
      <StatusChip type="success">success</StatusChip>
      <StatusChip type="success" selected>success</StatusChip>
      <StatusChip type="warning">warning</StatusChip>
      <StatusChip type="warning" selected>warning</StatusChip>
      <StatusChip type="danger">danger</StatusChip>
      <StatusChip type="danger" selected>danger</StatusChip>
    </div>
  </Story>
</Preview>



### CloseableChip

Props:
- onClose: function()

By default, there will be a close Icon on the right that can be customized by changing _renderRightIcon_


<Preview>
  <Story name="CloseableChip">
    <div style={{display: "flex", justifyContent:"space-around", alignItems: "center"}}>
      <CloseableChip onClose={() => console.log("close fired")}>
        with close
      </CloseableChip>
      <CloseableChip onClick={() => console.log("click")} onClose={() => console.log("close custom fired")}
      renderRightIcon={() => <ChipIcon name="arrow"/>}>
        with custom close
      </CloseableChip>
      <DerivedCloseableChip onClose={() => console.log("close custom fired")}>
        with custom style
      </DerivedCloseableChip>
    </div>
  </Story>
</Preview>

